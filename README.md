...
- Fix overlapping widgets in QSO Detail dialog: "QSLs Message" label and edit were placed in the same grid cell as "QSL Received via", moved them to their own row
...
- Fix QSL-via truncation: raise the `qslViaEdit` input `maxLength` from 25 to 50 in `NewContactWidget.ui` and `QSODetailDialog.ui`. Real QRZ `qslmgr` routing strings (mapped to `qsl_via`) frequently exceed 25 chars and were clipped by the UI before storage, even though the DB column is unlimited `TEXT` and ADIF sets no `QSL_VIA` length limit. [PR#1081](https://github.com/foldynl/QLog/pull/1081)
