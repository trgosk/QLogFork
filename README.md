...
- Fix QSL-via truncation: raise the `qslViaEdit` input `maxLength` from 25 to 50 in `NewContactWidget.ui` and `QSODetailDialog.ui`. Real QRZ `qslmgr` routing strings (mapped to `qsl_via`) frequently exceed 25 chars and were clipped by the UI before storage, even though the DB column is unlimited `TEXT` and ADIF sets no `QSL_VIA` length limit.
